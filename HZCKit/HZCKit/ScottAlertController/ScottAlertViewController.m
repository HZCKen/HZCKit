...
- (void)configureAlertViewWidth {
    if (!CGSizeEqualToSize(_alertView.frame.size, CGSizeZero)) {
        NSLog(@"%@", NSStringFromCGRect(_alertView.frame));
...
+ (void)showAlertViewWithMessage:(NSString *)message
                       targetVC :(UIViewController *)targetVC
                         handler:(void (^)(ScottAlertAction * _Nonnull))handler {
 
        
        ScottAlertView *alertView = [ScottAlertView alertViewWithTitle:@"提示" message:message];
        
        [alertView addAction:[ScottAlertAction actionWithTitle:@"确定" style:ScottAlertActionStyleNone handler:handler]];
...
+ (instancetype)showAlertViewCancelMessage:(NSString *)message
    ...
    [alertView addAction:[ScottAlertAction actionWithTitle:@"取消" style:ScottAlertActionStyleNone handler:handler]];
...
+ (void)showAlertViewWithViewController:(UIViewController *)targetVC
                           ...
                           buttonTitles:(NSArray *)buttonTitles
                                handler:(void (^)(NSInteger,ScottAlertAction *))handler {
    ScottAlertView *alertView = [ScottAlertView alertViewWithTitle:title message:message];
    
    for (int i = 0; i < buttonTitles.count; i++) {
        [alertView addAction:[ScottAlertAction actionWithTitle:buttonTitles[i] style:ScottAlertActionStyleNone handler:^(ScottAlertAction * _Nonnull action) {
            if (handler) {
                handler(i,action);
            }
...
+ (void)showAlertViewIncludeCancalWithMessage :(NSString *)message targetVC :(UIViewController *)targetVC handler:(void (^ __nullable)(ScottAlertAction *action))handler {
    ...
    [alertView addAction:[ScottAlertAction actionWithTitle:@"取消" style:ScottAlertActionStyleNone handler:nil]];
...
+ (instancetype)showAlertViewIncludeCancalWithMessage :(NSString *)message targetVC :(UIViewController *)targetVC sureHandler:(void (^ __nullable)(ScottAlertAction *action))sureHandler cancalHandler:(void(^ __nullable)(ScottAlertAction *action))cancelHandler {
    ...
    [alertView addAction:[ScottAlertAction actionWithTitle:@"取消" style:ScottAlertActionStyleNone handler:cancelHandler]];
    
    [alertView addAction:[ScottAlertAction actionWithTitle:@"确定" style:ScottAlertActionStyleNone handler:sureHandler]];
...
+ (void)showAlertViewIncludTextFieldWithTitle :(NSString *)title
    ...
    //title不要修改,如果修改后查找下项目有用到该方法的修改相对应的判断条件
    ScottAlertView *alertView = [ScottAlertView alertViewWithTitle:title message:message];
    [alertView addAction:[ScottAlertAction actionWithTitle:@"取消" style:ScottAlertActionStyleNone handler:^(ScottAlertAction *action) {
        
        handler(action,alertView);
        
    }]];
    
    [alertView addAction:[ScottAlertAction actionWithTitle:@"确定" style:ScottAlertActionStyleNone handler:^(ScottAlertAction *action) {
